...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		// Instruction HALT
		case HALT_INST: 
		if(Processor_PSW_BitState(EXECUTION_MODE_BIT) == 1) { //Comprueba si eres usuario
			Processor_ActivatePSW_Bit(POWEROFF_BIT);
		} else {
			Processor_RaiseInterrupt(EXCEPTION_BIT); //En caso de que no sacas excepcion
		}
		break;
	...
	int i;
		...
		for (i=0;i<INTERRUPTTYPES;i++)
			...
			if (Processor_GetInterruptLineStatus(i)) {
				...
				Processor_ActivatePSW_Bit(INTERRUPT_MASKED_BIT); //Se activa la mascara
